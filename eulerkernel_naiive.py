...
BLOCKSIZE_X = 512        #Number of param sets per block
NUM_STATES = 5

#Setting up grid of params to simulate with
a_gains = np.asarray([i * 0.2 for i in range(-64, 64)], dtype=np.float64)
b_params = np.asarray([i * 0.1 for i in range(-64, 64)], dtype=np.float64)
...
def get_noise(noise_array,
    ...
    for i in range(len(noise_array)):
        noise_array[i] = xoroshiro128p_normal_float64(RNG, firstidx + i) * sigmas[i]
    return noise_array
...
def naiive_euler_kernel(output,
    ...
    # Store filter coefficients in constant memory (good when all threads want the same value)
