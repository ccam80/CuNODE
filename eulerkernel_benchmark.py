...
a_gains = np.asarray([i * 0.2 for i in range(-16, 16)], dtype=np.float32)
b_params = np.asarray([i * 0.1 for i in range(-16, 16)], dtype=np.float32)
grid_params = np.asarray([(a, b) for a in a_gains for b in b_params])

#TODO: Consider scaling.
filtercoefficients = np.asarray(ds_100_filter, dtype=np.float32)
...
def cantilever_euler_kernel(output,
    ...
    for i in range(l_n_outer):
        ...
        for j in range(l_ds_rate):
            thread_noise = xoroshiro128p_normal_float32(RNG, idx) * noise_sigmas[tx]
        ...
        s_working[ty, 11, tx] = cuda.selp(tx in [0, 2, 3, 4], float32(0.0), s_working[ty, 11, tx])
        s_working[ty, 3, tx] = cuda.selp(tx == 1, float32(0.0), s_working[ty, 3, tx])
        cuda.syncwarp()
...
# durations = [500, 1000]
opt_timings = np.zeros((len(num_parallel), len(durations)))
for j, parallels in enumerate(num_parallel):
    for k, duration in enumerate(durations):
        ...
        if (fs * duration * (parallels * BLOCKSIZE_X + 1) * 4) > (7168 * 1024**2):
            opt_timings[j, k] = 0
        else:
            outputstates = cuda.pinned_array((int(fs * duration), len(sampled_combinations), BLOCKSIZE_X), dtype=np.float32)
            outputstates[:, :, :] = 0
            reference = np.sin(np.linspace(0,duration - step_size, int(duration / step_size)), dtype=np.float32)

            # BLOCKSIZE_Y = literally(ysize)
            ...
            print('run', j*len(durations) + k, ' out of ', len(durations) * len(num_parallel))
            opt_timings[j, k] = timing_nb_wall / 3 * 1000

            # print('numba events:', timing_nb / 3, 'ms')
            print('numba wall  :', timing_nb_wall / 3 * 1000, 'ms')
np.savetxt(f"opt_timings{time.time()}", opt_timings)